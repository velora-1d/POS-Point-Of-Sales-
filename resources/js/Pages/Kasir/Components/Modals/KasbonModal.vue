        ...
        >
            <div class="w-full max-w-xl rounded-3xl border border-stone-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xl text-stone-900 dark:text-slate-100">
                <div class="flex items-start justify-between gap-4 border-b border-stone-200/60 dark:border-slate-800/80 px-6 py-5">
                    <div>
                        <span class="rounded-full border border-amber-500/20 bg-amber-500/5 dark:bg-amber-500/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.22em] text-amber-600 dark:text-amber-300">
                            Kasbon / Piutang
                        </span>
                ...
                <div class="space-y-4 px-6 py-5">
                    <div class="rounded-2xl border border-stone-200 dark:border-slate-800 bg-stone-50/50 dark:bg-slate-950/60 p-4">
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-orange-600 dark:text-orange-400">Nama Pelanggan</p>
                                <p class="mt-2 text-sm font-bold text-stone-900 dark:text-white">
                                    {{ kasbonTargetOrder.customer?.name || 'Customer tidak ditemukan' }}
                                </p>
                                <p class="mt-1 text-xs text-stone-500 dark:text-slate-400">
                                    {{ kasbonTargetOrder.customer?.phone || 'Tanpa nomor HP' }}
                                </p>
                            </div>
                            <div class="text-right">
                                <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-orange-600 dark:text-orange-400">Sisa Tagihan</p>
                                <p class="mt-2 text-lg font-black text-amber-600 dark:text-amber-300">
                                    {{ formatPrice(Number(kasbonTargetOrder.total_amount) - Number(kasbonTargetOrder.paid_amount || 0)) }}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input
                                ...
                                type="date"
                                class="w-full rounded-xl border border-stone-200 dark:border-slate-800 bg-stone-50 dark:bg-slate-950 px-4 py-3 text-xs text-stone-900 dark:text-slate-200 placeholder-stone-400 dark:placeholder-slate-500 focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500"
                            />
                            <p class="mt-2 text-[11px] text-stone-500 dark:text-slate-500">
                ...
                <div class="flex items-center justify-end gap-3 border-t border-stone-200/60 dark:border-slate-800/80 px-6 py-4">
                    <button
                        @click="closeKasbonModal"
                        type="button"
                        class="rounded-2xl border border-stone-200 dark:border-slate-800 bg-stone-100 dark:bg-slate-950 px-4 py-3 text-xs font-bold text-stone-700 dark:text-slate-300 hover:bg-stone-200 dark:hover:bg-slate-900 active:scale-[0.98] transition"
                    >
                        Batal
                    </button>
                    <button
                    ...
                    >
                        {{ kasbonForm.processing ? 'Menyimpan Kasbon...' : 'Tutup Sebagai Kasbon' }}
                    </button>
                </div>
            </div>
        </div>
