...
<template>
    ...
    <div class="min-h-screen bg-[#0b1220] px-4 py-8 text-slate-100">
                ...
                <div class="rounded-[28px] border border-slate-800 bg-white text-slate-900 shadow-2xl print:border-0 print:shadow-none">
                    ...
                    <div class="space-y-6 px-8 py-8">
                        <div class="grid gap-3 text-sm text-slate-600 sm:grid-cols-2">
                            <div>
                                <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">Pelanggan</p>
                                <p class="mt-2 font-semibold text-slate-900">{{ order.customer?.name || 'Walk-in' }}</p>
                                <p class="mt-1">{{ order.customer?.phone || '-' }}</p>
                            </div>
                            <div>
                                <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">Kasir / Layanan</p>
                                ...
                                <tbody class="divide-y divide-slate-200">
                                    <tr v-for="item in order.items" :key="item.id">
                                        <td class="px-4 py-3">
                                            ...
                                            <div v-if="item.variant_name" class="mt-1 text-xs text-slate-500">
                        ...
                        <div v-if="order.payment_logs.length" class="rounded-2xl border border-slate-200 p-5">
                            <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">Riwayat Pembayaran</p>
                            <div class="mt-4 space-y-3">
                                ...
                                >
                                    <div>
                                        ...
                                        <p v-if="log.notes" class="mt-1 text-xs text-slate-500">
                                            {{ log.notes }}
                                        </p>
                                    </div>
                        ...
                        <div v-if="order.pre_order || order.kasbon" class="grid gap-4 sm:grid-cols-2">
                            <div v-if="order.pre_order" class="rounded-2xl border border-slate-200 p-4">
                                <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">Pre-Order</p>
                            ...
                            <div v-if="order.kasbon" class="rounded-2xl border border-slate-200 p-4">
                                <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">Kasbon</p>
                    ...
                    <section class="rounded-3xl border border-slate-800/80 bg-slate-900/80 p-6">
                        ...
                        <div class="mt-5 space-y-3">
                            ...
                            <button
                                type="button"
                                @click="skipReceipt"
                                class="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm font-bold text-slate-300 transition hover:border-slate-600"
                            >
                                Lewati Struk
                            </button>
                        </div>
                    ...
                    <section class="rounded-3xl border border-slate-800/80 bg-slate-900/80 p-6">
                        ...
                        <div class="mt-4 space-y-3 text-sm text-slate-300">
                            <div class="flex justify-between">
                                <span class="text-slate-500">Metode terakhir</span>
                                <span class="font-semibold text-white">{{ formatPaymentMethod(order.payment_method) }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-slate-500">Status payment</span>
                                <span class="font-semibold text-white">{{ order.payment_status || '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-slate-500">Receipt method</span>
                                <span class="font-semibold text-white">{{ order.receipt_method }}</span>
                            </div>
                        </div>
                    </section>
                </div>
