...
function markReceipt(method: 'print' | 'whatsapp' | 'skip') {
    router.post(route('transactions.receipt.mark', props.order.id), {
        receipt_method: method,
        receipt_phone: props.order.receipt_phone || props.order.customer?.phone || null,
    }, {
        preserveScroll: true,
        preserveState: true,
    });
...
<template>
    ...
    <div class="min-h-screen bg-[#0b1220] px-4 py-8 text-stone-900 dark:text-slate-100">
        <div class="mx-auto flex max-w-5xl flex-col gap-6">
            ...
            <div class="flex items-center justify-between gap-4 print:hidden">
                <div>
                    ...
                    <h1 class="mt-2 text-2xl font-black text-stone-900 dark:text-white">
Struk {{ order.order_number }}
                    </h1>
                    <p class="mt-1 text-sm text-stone-500 dark:text-slate-400">
                        Preview struk transaksi, pre-order, atau kasbon sebelum print / kirim WhatsApp.
                    </p>
                </div>
                <Link
            ...
            <div class="grid gap-6 xl:grid-cols-[0.72fr_0.28fr]">
                <div :class="[receiptFontClass, receiptPaperClass]" class="shadow-2xl print:border-0 print:shadow-none transition-all duration-300">
                    <div :class="receiptDividerClass" class="pb-6 text-center">
                        ...
                        <p class="mt-1.5 text-sm text-stone-400 dark:text-slate-500">{{ outlet.address || '-' }}</p>
                        <p class="mt-0.5 text-sm text-stone-400 dark:text-slate-500">{{ outlet.phone || '-' }}</p>
                        <p class="mt-3 text-xs font-bold uppercase tracking-[0.22em] text-stone-500 dark:text-slate-400">
                    ...
                    <div class="space-y-6 px-8 py-8">
                        <div class="grid gap-3 text-sm text-slate-600 sm:grid-cols-2">
                            <div>
                                <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-stone-500 dark:text-slate-400">Pelanggan</p>
                                <p class="mt-2 font-semibold text-slate-900">{{ order.customer?.name || 'Walk-in' }}</p>
                                <p class="mt-1">{{ order.customer?.phone || '-' }}</p>
                            </div>
                            <div>
                                <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-stone-500 dark:text-slate-400">Kasir / Layanan</p>
                                <p class="mt-2 font-semibold text-slate-900">{{ order.cashier || 'Kasir Restoran' }}</p>
                                <p class="mt-1">{{ order.table || 'Takeaway / Counter' }}</p>
                            </div>
                        </div>

                        <div class="overflow-hidden rounded-2xl border border-slate-200">
                            <table class="min-w-full divide-y divide-slate-200 text-left text-sm">
                                <thead class="bg-slate-50 text-[10px] font-bold uppercase tracking-[0.18em] text-stone-400 dark:text-slate-500">
                                    <tr>
                                ...
                                <tbody class="divide-y divide-slate-200">
                                    <tr v-for="item in order.items" :key="item.id">
                                        <td class="px-4 py-3">
                                            ...
                                            <div v-if="item.variant_name" class="mt-1 text-xs text-stone-400 dark:text-slate-500">
                                        ...
                                        <td class="px-4 py-3 text-right font-bold text-slate-900">{{ formatCurrency(item.total_price) }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        ...
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                            ...
                            <div class="mt-2 flex justify-between text-sm" :class="order.remaining_amount > 0 ? 'text-orange-600' : 'text-emerald-700'">
                                <span>Sisa</span>
                                <span class="font-bold">{{ formatCurrency(order.remaining_amount) }}</span>
                            </div>
                        </div>

                        <div v-if="order.payment_logs.length" class="rounded-2xl border border-slate-200 p-5">
                            <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-stone-500 dark:text-slate-400">Riwayat Pembayaran</p>
                                ...
                                >
                                    <div>
                                        <p class="text-sm font-semibold text-slate-900">
                                            {{ log.payment_type.replace('_', ' ') }} · {{ formatPaymentMethod(log.payment_method) }}
                                        </p>
                                        <p class="mt-1 text-xs text-stone-400 dark:text-slate-500">
                                            {{ formatDateTime(log.created_at) }} · {{ log.user_name || 'System' }}
                                        </p>
                                        <p v-if="log.notes" class="mt-1 text-xs text-stone-400 dark:text-slate-500">
                                            {{ log.notes }}
                                        </p>
                                    </div>
                                    <span class="text-sm font-black text-slate-900">{{ formatCurrency(log.amount) }}</span>
                                </div>
                            </div>
                        </div>

                        <div v-if="order.pre_order || order.kasbon" class="grid gap-4 sm:grid-cols-2">
                            <div v-if="order.pre_order" class="rounded-2xl border border-slate-200 p-4">
                                <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-stone-500 dark:text-slate-400">Pre-Order</p>
                                <p class="mt-2 text-sm font-semibold text-slate-900">Pickup {{ formatDateTime(order.pre_order.pickup_at) }}</p>
                                <p class="mt-1 text-xs text-stone-400 dark:text-slate-500">
                                    DP {{ order.pre_order.down_payment_type === 'percentage' ? `${order.pre_order.down_payment_value}%` : formatCurrency(order.pre_order.down_payment_value || 0) }}
                                </p>
                            </div>
                            <div v-if="order.kasbon" class="rounded-2xl border border-slate-200 p-4">
                                <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-stone-500 dark:text-slate-400">Kasbon</p>
                                <p class="mt-2 text-sm font-semibold text-slate-900">Jatuh tempo {{ order.kasbon.due_date ? formatDateTime(order.kasbon.due_date) : '-' }}</p>
                                <p class="mt-1 text-xs text-stone-400 dark:text-slate-500">
                        ...
                        <div class="pt-4 mt-2 text-center text-xs text-stone-400 dark:text-slate-500 italic leading-relaxed" :class="receiptDividerClass">
                            {{ receiptFooterText }}
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6 print:hidden">
                    <section class="rounded-3xl border border-stone-200 dark:border-slate-800/80 bg-stone-50 dark:bg-slate-900/80 p-6">
                        <p class="text-[10px] font-bold uppercase tracking-[0.22em] text-orange-300">Aksi Struk</p>
                        <div class="mt-5 space-y-3">
                            ...
                            <button
                                type="button"
                                @click="skipReceipt"
                                class="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-stone-200 dark:border-slate-700 bg-stone-100 dark:bg-slate-950 px-4 py-3 text-sm font-bold text-stone-600 dark:text-slate-300 transition hover:border-stone-300 dark:border-slate-600"
                            >
                                Lewati Struk
                            </button>
                        </div>
                    ...
                    <section class="rounded-3xl border border-stone-200 dark:border-slate-800/80 bg-stone-50 dark:bg-slate-900/80 p-6">
                        <p class="text-[10px] font-bold uppercase tracking-[0.22em] text-orange-300">Ringkasan Payment</p>
                        <div class="mt-4 space-y-3 text-sm text-stone-600 dark:text-slate-300">
                            <div class="flex justify-between">
                                <span class="text-stone-400 dark:text-slate-500">Metode terakhir</span>
                                <span class="font-semibold text-stone-900 dark:text-white">{{ formatPaymentMethod(order.payment_method) }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-stone-400 dark:text-slate-500">Status payment</span>
                                <span class="font-semibold text-stone-900 dark:text-white">{{ order.payment_status || '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-stone-400 dark:text-slate-500">Receipt method</span>
                                <span class="font-semibold text-stone-900 dark:text-white">{{ order.receipt_method }}</span>
                            </div>
                        </div>
                    </section>
                </div>
