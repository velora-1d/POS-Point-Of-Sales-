...
import { CheckCircle2, Percent, Receipt, Shield } from '@lucide/vue';
...
const form = useForm({
    ...
    manual_discount_enabled: Boolean(props.formDefaults.manual_discount_enabled),
    manual_discount_threshold: String(props.formDefaults.manual_discount_threshold || 50000),
    order_edit_enabled: Boolean(props.formDefaults.order_edit_enabled),
    order_edit_threshold: String(props.formDefaults.order_edit_threshold || 150000),
});
...
watch(
    ...
    (defaults) => {
        form.defaults({
            ...
            manual_discount_threshold: String(defaults.manual_discount_threshold || 50000),
            order_edit_enabled: Boolean(defaults.order_edit_enabled),
            order_edit_threshold: String(defaults.order_edit_threshold || 150000),
        });
            ...
            <div class="flex flex-col gap-2">
                <div>
                    ...
                    <p class="mt-1 text-sm text-stone-500 dark:text-slate-400">
                        Definisikan threshold nominal yang mewajibkan approval owner untuk diskon manual dan edit order.
                    </p>
                </div>
            </div>
        </template>
                ...
                <section class="rounded-3xl border border-stone-200 dark:border-slate-800/80 bg-stone-50 dark:bg-slate-900/80 p-6">
                    <div class="flex flex-col gap-4 border-b border-stone-200 dark:border-slate-800/80 pb-5 lg:flex-row lg:items-end lg:justify-between">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-stone-500 dark:text-slate-400">
                                Jika nominal melebihi batas, kasir wajib mengisi PIN owner sebelum aksi diproses.
                            </p>
                        </div>
                    ...
                    <div class="mt-6 space-y-5">
                        <div class="rounded-3xl border border-stone-200 dark:border-slate-800 bg-white dark:bg-slate-950/50 p-5">
                            <div class="flex items-start justify-between gap-4">
                                <div>
                                    <h4 class="text-base font-black text-stone-900 dark:text-white">
                                        Manual Discount Threshold
                                    </h4>
                                    <p class="mt-1 text-sm leading-6 text-stone-500 dark:text-slate-400">
                                        Berlaku saat kasir memakai voucher atau diskon manual. Sistem menghitung nominal diskon manual yang benar-benar applied.
                                    </p>
                                </div>
                                <button
                                    type="button"
                                    @click="form.manual_discount_enabled = !form.manual_discount_enabled"
                                    class="rounded-full border px-3 py-1 text-[11px] font-bold uppercase tracking-wider"
                                    :class="form.manual_discount_enabled ? 'border-emerald-500/20 bg-emerald-500/10 text-emerald-300' : 'border-stone-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-stone-500 dark:text-slate-400'"
                                >
                                    {{ form.manual_discount_enabled ? 'Aktif' : 'Nonaktif' }}
                                </button>
                            </div>
                            ...
                            <div class="mt-5">
                                ...
                                <p class="mt-2 text-xs leading-5 text-stone-400 dark:text-slate-500">
                                    Preview limit: {{ formatCurrency(form.manual_discount_threshold) }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="rounded-3xl border border-stone-200 dark:border-slate-800 bg-white dark:bg-slate-950/50 p-5">
                            <div class="flex items-start justify-between gap-4">
                                <div>
                                    ...
                                    <p class="mt-1 text-sm leading-6 text-stone-500 dark:text-slate-400">
                                        Berlaku saat kasir mengubah order. Jika total order hasil edit melewati batas, sistem mewajibkan PIN owner.
                                    </p>
                                </div>
                                <button
                                    type="button"
                                    @click="form.order_edit_enabled = !form.order_edit_enabled"
                                    class="rounded-full border px-3 py-1 text-[11px] font-bold uppercase tracking-wider"
                                    :class="form.order_edit_enabled ? 'border-emerald-500/20 bg-emerald-500/10 text-emerald-300' : 'border-stone-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-stone-500 dark:text-slate-400'"
                                >
                                    {{ form.order_edit_enabled ? 'Aktif' : 'Nonaktif' }}
                                </button>
                            </div>
                            ...
                            <div class="mt-5">
                                ...
                                />
                                <p class="mt-2 text-xs leading-5 text-stone-400 dark:text-slate-500">
                                    Preview limit: {{ formatCurrency(form.order_edit_threshold) }}
                                </p>
                            </div>
                        </div>
                    </div>

                    <div class="mt-6 flex justify-end border-t border-stone-200 dark:border-slate-800/80 pt-5">
                        <button
                            type="button"
                            @click="submitSave"
                        ...
                        >
                            {{ form.processing ? 'Menyimpan...' : 'Simpan Approval Rules' }}
                        </button>
                    </div>
                </section>

                <section class="space-y-6">
                    <div class="rounded-3xl border border-stone-200 dark:border-slate-800/80 bg-stone-50 dark:bg-slate-900/80 p-6">
                        <p class="text-[10px] font-bold uppercase tracking-[0.22em] text-orange-300">
                            Cara Kerja
                        </p>
                        <h3 class="mt-2 text-lg font-black text-stone-900 dark:text-white">
                        ...
                        <p class="mt-3 text-sm leading-7 text-stone-500 dark:text-slate-400">
                            Runtime order sekarang mengecek threshold sebelum diskon manual atau edit order disimpan. Jika limit terlampaui, kasir wajib memasukkan PIN owner yang valid.
                        </p>
                    </div>
                    ...
                    <div class="rounded-3xl border border-stone-200 dark:border-slate-800/80 bg-stone-50 dark:bg-slate-900/80 p-6">
                        ...
                        <div class="mt-4 space-y-4">
                            <div class="rounded-2xl border border-stone-200 dark:border-slate-800 bg-white dark:bg-slate-950/50 p-4">
                                <p class="text-sm font-bold text-stone-900 dark:text-white">Diskon Manual</p>
                                <p class="mt-2 text-xs leading-6 text-stone-500 dark:text-slate-400">
                                    Diterapkan pada create order baru dan settlement tagihan existing ketika voucher manual dipakai.
                                </p>
                            </div>
                            <div class="rounded-2xl border border-stone-200 dark:border-slate-800 bg-white dark:bg-slate-950/50 p-4">
                                <p class="text-sm font-bold text-stone-900 dark:text-white">Edit Order</p>
                                <p class="mt-2 text-xs leading-6 text-stone-500 dark:text-slate-400">
                                    Diterapkan saat total order hasil edit mencapai atau melewati nominal threshold outlet.
                                </p>
                            </div>
                            <div class="rounded-2xl border border-stone-200 dark:border-slate-800 bg-white dark:bg-slate-950/50 p-4">
                                <p class="text-sm font-bold text-stone-900 dark:text-white">PIN Supervisor Lama</p>
                                <p class="mt-2 text-xs leading-6 text-stone-500 dark:text-slate-400">
                                    Untuk order status <code class="rounded bg-stone-100 dark:bg-slate-950 px-1.5 py-0.5 text-[11px] text-orange-300">in_progress</code>, rule supervisor tetap berlaku bila threshold owner tidak terpenuhi.
                                </p>
                            </div>
                        </div>
                    </div>
